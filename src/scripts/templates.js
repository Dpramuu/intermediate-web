...
export function generateReportItemTemplate({
  ...
  const location = latitude && longitude ? `${latitude}, ${longitude}` : 'Lokasi tidak tersedia';
  
  return `
    <div tabindex="0" class="report-item" data-reportid="${id}">
      <img class="report-item__image" src="${evidenceImages?.[0] || 'images/placeholder-image.jpg'}" alt="${title || 'Untitled'}">
      <div class="report-item__body">
        <div class="report-item__main">
          <h2 id="report-title" class="report-item__title">${title || 'Untitled'}</h2>
          <div class="report-item__more-info">
            <div class="report-item__createdat">
              <i class="fas fa-calendar-alt"></i> ${showFormattedDate(createdAt || new Date(), 'id-ID')}
            </div>
        ...
        <div class="report-item__more-info">
          <div class="report-item__author">
            Dilaporkan oleh: ${reporterName || 'Unknown'}
          </div>
        </div>
