...
export default class ReportDetailPage {
  ...
  async initialMap(latitude, longitude, title) {
    try {
      const { createMap, createSingleMarker } = await import('../../utils/map');
      const map = createMap('map');
      if (latitude && longitude) {
        createSingleMarker(map, latitude, longitude, `<strong>${title}</strong>`);
      }
    } catch (error) {
