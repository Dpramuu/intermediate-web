...
export default class NewPage {
  ...
  async initialMap() {
    const map = await import('../../utils/map');
    const leafletMap = map.createMap('map');

    const updateFormCoordinates = (lat, lng) => {
      this.#form.elements.namedItem('latitude').value = parseFloat(lat).toFixed(6);
      this.#form.elements.namedItem('longitude').value = parseFloat(lng).toFixed(6);
    };

    const latitude = this.#form.elements.namedItem('latitude').value;
    const longitude = this.#form.elements.namedItem('longitude').value;
    map.setMarker(latitude, longitude, updateFormCoordinates);

    leafletMap.on('click', (e) => {
      const { lat, lng } = e.latlng;
      updateFormCoordinates(lat, lng);
      map.setMarker(lat, lng, updateFormCoordinates);
    });

    this.#form.elements.namedItem('latitude').addEventListener('change', (e) => {
      const lat = e.target.value;
      const lng = this.#form.elements.namedItem('longitude').value;
      if (!isNaN(lat) && !isNaN(lng)) {
        map.setMarker(lat, lng, updateFormCoordinates);
      }
    });

    this.#form.elements.namedItem('longitude').addEventListener('change', (e) => {
